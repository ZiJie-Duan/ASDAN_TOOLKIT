...
class CORE_CONTROL():
    ...
    def table_writer_gui(self):
        '''
        gui简易版 表格写入器
        '''
        print("[table_writer] GUI easy writer")
        data = {}
        data["file_path"] = self.cmdlist[0]
        data["shells"] = []
...
class CORE_GUI_CONTROL(QMainWindow,Ui_mainWindow):
    '''
    GUI 控制的类
    '''
    input_file_path_Signal = pyqtSignal(str)
    output_file_path_Signal = pyqtSignal(str)
    path_show_data= pyqtSignal(str,str)
    #core_ana_mode = pyqtSignal(str)

    def __init__(self,parent=None):
        super(CORE_GUI_CONTROL,self).__init__(parent)
        self.setupUi(self)
        self.initUI()
        self.core = None
        self.input_file_path_v = None
        self.out_file_path_v = None


    def initUI(self):
        jpg = QtGui.QPixmap(r"C:\Users\lucyc\Desktop\ASDAN_TOOLKIT\test\client\json.jpg").scaled(self.JASONface.width(), self.JASONface.height())
        ...
        self.path_show_data.connect(self.show_path)
        self.tra_select_buttom.clicked.connect(self.TRA_read_table)
        self.ana_select_buttom.clicked.connect(self.ANA_core)
        self.tsr_select_buttom.clicked.connect(self.TSR_table_style)
        self.save_data.clicked.connect(self.Creat_table)

    def get_in_file_path(self):
        filePath, _ = QFileDialog.getOpenFileName(self, "open file", "", "*.xlsx;;All Files(*)")
        self.path_show_data.emit("in",filePath)

    def get_out_file_path(self):
        filePath, _ = QFileDialog.getSaveFileName(self, "save file", "", "*.xls;;All Files(*)")
        self.path_show_data.emit("out",filePath)

    def show_path(self,mods,data):
        if mods == "in":
            self.input_file_path.setText(data)
            self.input_file_path_v = data
        elif mods == "out":
            self.output_file_path.setText(data)
            self.out_file_path_v = data

    def TRA_read_table(self):
        self.core.cmdlist = ["1"]
        self.core.init_data_structure()
        cmdlist = [self.tra_select.currentText(),self.input_file_path_v]
        self.core.cmdlist = cmdlist
        self.core.read_table()

    def ANA_core(self):
        self.core.cmdlist = [self.ana_select.currentText()]
        print(self.ana_select.currentText())
        self.core.analyzer()

    def TSR_table_style(self):
        self.core.cmdlist = [self.tsr_select.currentText()]
        self.core.table_style_rendering()

    def Creat_table(self):
        self.core.cmdlist = [self.out_file_path_v]
        self.core.table_writer_gui()

if __name__=="__main__":
    core = CORE_CONTROL()
    app = QApplication(sys.argv)
    win = CORE_GUI_CONTROL()
    win.core = core
    win.show()
    sys.exit(app.exec_())
